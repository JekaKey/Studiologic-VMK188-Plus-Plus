...
void MSC_BOT_DataOut (USB_OTG_CORE_HANDLE  *pdev, 
                      uint8_t epnum)
{
  PRINTF("     MSC_BOT_DataOut, epnum: %d, MSC_BOT_State: %d \n",epnum, MSC_BOT_State);
...
/**
* @brief  MSC_BOT_CBW_Decode CWB - Command Block Wrapper
...
static void  MSC_BOT_CBW_Decode (USB_OTG_CORE_HANDLE  *pdev)
{
  ...
  else
  {
             ...
             (MSC_BOT_State != BOT_LAST_DATA_IN)) 
    {
      ...
      {
     	 PRINTF("    MSC_BOT_CBW_Decode, MSC_BOT_State: %d \n",MSC_BOT_State);
                              ...
                              uint16_t len)
{
  ...
  PRINTF("    MSC_BOT_SendData, len: %d , buff: %d,%d,%d,%d,%d,%d,%d,%d\n", len, buf[0],buf[1],buf[2],buf[3],buf[4],buf[5],buf[6],buf[7]);

  DCD_EP_Tx (pdev, MSC_IN_EP, buf, len);
  
  PRINTF("    MSC_BOT_SendData, DCD_EP_Tx() finished\n");
}
                              ...
                              uint8_t CSW_Status)
{
  ...
  MSC_BOT_State = BOT_IDLE;
  PRINTF("    MSC_BOT_SendCSW, CSW_Status: %d \n", CSW_Status);
