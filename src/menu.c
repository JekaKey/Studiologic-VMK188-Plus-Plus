...
static void check_saving_calibr(void);
static void check_saving_curve(void);
...
MAKE_MENU(menu_clb_edit,	menu_clb_save,		NULL_ENTRY,			NULL_ENTRY,		menu_slider1,	0,		NULL,	t_none,		0,		0,		menu_preset_sl_enter,			check_saving_calibr,	"Edit calibr."	);
MAKE_MENU(menu_clb_save,	menu_clb_copy,		menu_clb_edit,		NULL_ENTRY,		NULL_ENTRY,		0,		NULL,	t_none,		0,		0,		startMenuYN_calibration_save,	check_saving_calibr,	"Save calibr."	);
MAKE_MENU(menu_clb_copy,	menu_clb_rename,	menu_clb_save,		NULL_ENTRY,		NULL_ENTRY,		0,		NULL,	t_none,		0,		0,		menu_calibration_copy,			check_saving_calibr,	"Copy calibr."	);
MAKE_MENU(menu_clb_rename,	menu_clb_del,		menu_clb_copy,		NULL_ENTRY,		NULL_ENTRY,		0,		NULL,	t_none,		0,		0,		menu_calibration_rename,		check_saving_calibr,	"Rename calibr.");
MAKE_MENU(menu_clb_del,		NULL_ENTRY,			menu_clb_rename,	NULL_ENTRY,		NULL_ENTRY,		1,		NULL,	t_none,		0,		0,		startMenuYN_calibration_delete,	check_saving_calibr,	"Delete calibr.");
...
MAKE_MENU(menu3_item1,		menu3_item2,		NULL_ENTRY,			NULL_ENTRY,		NULL_ENTRY,		0,		NULL,	t_none,		0,		0,		menu_edit_curve,				check_saving_curve,		"Edit curve"	);
MAKE_MENU(menu3_item2,		menu3_item3,		menu3_item1,		NULL_ENTRY,		NULL_ENTRY,		0,		NULL,	t_none,		0,		0,		startMenuYN_curve_save,			check_saving_curve,		"Save curve"	);
MAKE_MENU(menu3_item3,		menu3_item4,		menu3_item2,		NULL_ENTRY,		NULL_ENTRY,		0,		NULL,	t_none,		0,		0,		menu_curve_copy,				check_saving_curve,		"Copy curve"	);
MAKE_MENU(menu3_item4,		menu3_item5,		menu3_item3,		NULL_ENTRY,		NULL_ENTRY,		0,		NULL,	t_none,		0,		0,		menu_curve_rename,				check_saving_curve,		"Rename curve"	);
MAKE_MENU(menu3_item5,		NULL_ENTRY,			menu3_item4,		NULL_ENTRY,		NULL_ENTRY,		1,		NULL,	t_none,		0,		0,		startMenuYN_curve_delete,		check_saving_curve,		"Delete curve"	);
...
MAKE_MENU(menu_pst_transp,	menu_pst_oct,	menu_pst_chan,	menu_pst_gen,	NULL_ENTRY,		0,		&Preset.Transpose,			t_int8,-TRANSPOSE_MAX,TRANSPOSE_MAX,NULL,					NULL,					"Transpose: "	);
...
MAKE_MENU(menu_pst_noff7,	menu_pst_trbtns,menu_pst_noff1,	menu_pst_gen,	NULL_ENTRY,		0,		&Preset.NoteOffDelay127,	t_uint16,	0,			999,		NULL,					NULL,					"OffDelayF: "	);
...
static void startMenuYN_preset_save(void) {
	if (Preset.Crc == getCRC(&Preset, sizeof(presetType))) {
...
static void startMenuYN_preset_copy(void) {
	...
	if (preset_save(path, &Preset) != FIO_OK)
		set_errIO(ERRIO_PRESETSAVE);
	if (SDFS_scandir("0:/" PRESET_DIR_NAME, &presets_list) != SDFS_OK)
		set_errIO(ERRIO_SCANDIR); //update Presets list
...
static void startMenuYN_calibration_save(void) {
	if (Calibration.Crc == getCRC(&Calibration, sizeof(calibrationType))) {
		showTempMessage("Calibration", "has not changes!");
		return;
	}
...
static void startMenuYN_curve_save(void) {
	if (Curve.Crc == getCRC(&Curve, sizeof(curve_points_type))) {
		showTempMessage("Curve", "has not changes!");
		return;
	}
...
static void menu_calibration_delete_yes(void){
	...
	if (calibration_delete(&calibrations_list) != FIO_OK)
		set_errIO(ERRIO_CALIBRATIONDELETE);
	if (SDFS_scandir("0:/" CALIBR_DIR_NAME, &calibrations_list) != SDFS_OK)
		set_errIO(ERRIO_SCANDIR);
	file_list_find(&calibrations_list, old_active_calibration_name);
	calibrations_list.active = calibrations_list.pos;
...
static void preset_name_current_state(void) {
	...
	if (currentState_save() != FIO_OK)
		set_errIO(ERRIO_CURRENTSTATESAVE);
}


static uint8_t leftButtonPressed, rightButtonPressed;

static void presets_button_handler(uint8_t event){
	uint8_t button=event & 0x7f;
    if ((event&0x80) && (button!=BUTTON_LEFT) && (button!=BUTTON_RIGHT)) //handle depress event only for Left and Right buttons
		 return;
	switch (button) {
	case MES_REDRAW:
	...
	case BUTTON_LEFT:
		...
		leftButtonPressed = 1;
		if (rightButtonPressed) { //if both buttons are pressed send to 0
			if (Preset.TransportBtnFunc == 2) {
				sendPanic(&Preset);
				Preset.OctaveShift = 0;
				octave_shift_show();
	...
	case BUTTON_RIGHT:
		...
		if (leftButtonPressed) {//if both buttons are pressed send to 0
			if (Preset.TransportBtnFunc == 2) {
					sendPanic(&Preset);
					Preset.OctaveShift=0;
					octave_shift_show();
...
static void calibrations_button_handler(uint8_t button){
	...
	case BUTTON_PAGEUP:
		...
		if(calibration_load(calibrations_list.names[calibrations_list.pos], &Calibration)!=FIO_OK)
			set_errIO(ERRIO_CALIBRATIONLOAD);
		show_calibration(&Calibration, &calibrations_list);
		break;
	case ENCODER_RIGHT1:
	case ENCODER_RIGHT2:
...
void menu_btns_n_msg_handler(uint8_t event) {
	...
	if (event & 0x80){
      if (I_state!=STATE_presets_list) //Button depressed only for State_presets_list
    	  return;
      if (Preset.TransportBtnFunc==0)// If <>buttons work as transport no reaction on depress
    	  return;
	}
	switch (I_state) {
...
static void check_saving_calibr(void) {
	if (Calibration.Crc != getCRC(&Calibration, sizeof(calibrationType))) {
		selectedMenuYNItem = (menuYNItem_type*) &menuYN_calibr_changes;
		toYNMenu();
	} else {
...
static void check_saving_curve(void) {
	if (Curve.Crc != getCRC(&Curve, sizeof(curve_points_type))) {
		selectedMenuYNItem = (menuYNItem_type*) &menuYN_curve_changes;
		toYNMenu();
	} else {
		curvelist_start();
	}
